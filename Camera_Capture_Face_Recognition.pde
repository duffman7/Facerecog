...
void draw() {
 ...
 // image(img, 0, height, img.width, img.height);
  ...
  for (int i = 0; i < faces.length; i++) {
    ...
    fill(#2d8fc8);
    rect(width-width/5.15, height/2, height, width);
    fill(255);
    textSize(35);
    text("CHECK FOR YOUR", width-width/2.5, height/5 );
    text("CELEBRITY DRUNKALIKE...", width-width/2.5, height/4);
    
   
    textSize(25);
    fill(255);
    text(display, width-width/2.5, height/3);
    
    text(">0.5 = C'MON IN!", width-width/2.5, height-height/4);
